...
public class JdbcUserDao implements UserDao {
     ...
     */
    @Override
    public User saveUser(String userName, String password, String role, boolean oneTimePass) {
        byte[] salt = passwordHasher.generateRandomSalt();
        String hashedPassword = passwordHasher.computeHash(password, salt);
        String saltString = new String(Base64.encode(salt));
        long newId = jdbcTemplate.queryForObject(
                "INSERT INTO users(username, password, salt, role, onetimepass) VALUES (?, ?, ?, ?, ?) RETURNING id", Long.class,
                userName, hashedPassword, saltString, role, oneTimePass);

        User newUser = new User();
        newUser.setId(newId);
        newUser.setUsername(userName);
        newUser.setRole(role);
        newUser.setOneTimePass(oneTimePass);
    ...
    @Override
    public void updateUserRole(String username, String updatedRole) {
    	String update = "UPDATE users SET role = ? WHERE username = ?";
    	jdbcTemplate.update(update, updatedRole, username);
    }
