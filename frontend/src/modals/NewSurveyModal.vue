<template>
  <div>
    ...
    <form v-if="showForm === true">
      ...
      <input name="user" type="text" v-model="user.username" disabled class="form-control" />
...
export default {
  ...
  methods: {
    ...
    parsePace(response) {
      if (response.pace == "Way too slow") {
        response.pace = 1;
      } else if (response.pace == "A little too slow") {
        response.pace = 2;
      } else if (response.pace == "Just right") {
        response.pace = 3;
      } else if (response.pace == "A little too fast") {
        response.pace = 4;
      } else if (response.pace == "Way too fast") {
        response.pace = 5;
      } else {
        response.pace = 0;
      }
    ...
    parseInterest(response) {
      if (response.interest == "Mind-numbingly boring") {
        response.interest = 1;
      } else if (response.interest == "Somewhat uninteresting") {
        response.interest = 2;
      } else if (response.interest == "Mostly interesting") {
        response.interest = 3;
      } else if (response.interest == "Very interesting") {
        response.interest = 4;
      } else if (
        response.interest ==
        "I couldn't sleep last night because I was too excited about what I just learned!"
      ) {
        response.interest = 5;
      } else {
        response.interest = 0;
    ...
    parseUnderstanding(response) {
      if (
        ...
        "Non-existent, I might as well have skipped class and spent the day at the bar"
      ) {
        response.understanding = 1;
      } else if (
        response.understanding ==
        "Poor, I understood some things but feel like a lot of it is still fuzzy"
      ) {
        response.understanding = 2;
      } else if (
        response.understanding ==
        "Fair, I have at least a general understanding of everything discussed and just need more practice to master the concepts"
      ) {
        response.understanding = 3;
      } else if (
        response.understanding ==
        "Good, I am confident I understand and can apply the presented material"
      ) {
        response.understanding = 4;
      } else if (
        response.understanding == "Outstanding, I could teach yesterday's class"
      ) {
