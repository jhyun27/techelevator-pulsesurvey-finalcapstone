    ...
    <div class="details" @mouseover="hover = true" @mouseleave="hover = false">
      <div class="meta-info">
        <div class="classroom-info">
          <h3>{{ survey.section.sectionName }}</h3>
          <h3>Instructor: {{ survey.instructor.firstName }} {{survey.instructor.lastName}}</h3>
        </div>
        <div class="topic-info">
          <h2>{{ survey.topic.topicName }}</h2>
        </div>
        <div class="location-info">
          <h3>Cohort [{{ survey.cohort.cohortNumber }}]</h3>
          <h3>{{ survey.location.city }}, {{survey.location.state}}</h3>
        </div>
      </div>

      <question-card :surveyResponseSet="survey.surveyResponses" />

      <div class="card-footer-container">
        <div class="card-footer">
          <h4>Survey Id: {{ survey.surveyId }}</h4>
          <h4>Version Id: {{ survey.versionId }}</h4>
          <h4>Last Edited: {{ timestamp }}</h4>
          <h4>Last Edited By: {{ survey.user.username }}</h4>
        </div>
      </div>
      <div v-show="hover == true" v-if="!dontShowButtons" class="card-header buttons">
        <v-btn small class="link" :to="{name :'survey', params : {surveyId : survey.surveyId }}">
          View This Survey's Details
          <v-icon>mdi-details</v-icon>
        </v-btn>

        <v-btn v-if="isAdminUser" small type="button" class="btn" @click="showModal(survey)">
          Edit Survey
